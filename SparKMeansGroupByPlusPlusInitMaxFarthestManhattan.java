...
/**
 * Spark InitMaxFarthestManhattan k-means
 */
public final class SparKMeansGroupByPlusPlusMaxInitDESCFarthestManhattan {
...
//            List<String> init_center = data.takeSample(false, 1);
            ...
            for (int i = 1; i < Point.k; i++) {
                ...
                JavaPairRDD<Double, String> data_distance_center = data.mapToPair(((PairFunction<String, Double, String>) p -> {
                    ...
                    //calculate each data min_distance of center
                    return new Tuple2<>(min_distance, p);
                }));

                //choose the farthest data as center
                JavaPairRDD<Double, String> distance_order = data_distance_center.sortByKey(false);
                String new_center = distance_order.take(1).get(0)._2();

                //add the selectd center into init_center
